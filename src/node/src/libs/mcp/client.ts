...
export const createMcpClient = async () => {
  ...
  const commandModule = await dynamicImport('../agent-infra/mcp-servers/commands/server.js');
  const fsModule = await dynamicImport('../agent-infra/mcp-servers/filesystem/server.js');
  const browserModule = await dynamicImport('../agent-infra/mcp-servers/browser/server.js');
